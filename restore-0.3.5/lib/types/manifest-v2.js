...
import { parseGameplayManifest } from "./gameplay.js";
...
export const PET_RENDERER_KINDS = ['sprite2d', 'live2d', 'frames2d'];
...
function safeFrameName(raw) {
    if (typeof raw !== 'string' || raw.trim() === '')
        return undefined;
    const value = raw.trim();
    if (value.includes('/') || value.includes('\\') || !PATH_SEGMENT_PATTERN.test(value))
        return undefined;
    const dot = value.lastIndexOf('.');
    if (dot <= 0)
        return undefined;
    if (!FRAMES2D_IMAGE_EXTENSIONS.has(value.slice(dot).toLowerCase()))
        return undefined;
    return value;
}
function parseFrames2dTrack(raw, field, diag) {
    ...
    if (extra.length > 0)
        diag.error(field + ': unknown field(s) ' + extra.map(k => JSON.stringify(k)).join(', '));
    const track = {};
    if (raw.frames !== undefined) {
        if (!Array.isArray(raw.frames) || raw.frames.length === 0) {
            ...
            for (const entry of raw.frames) {
                ...
                if (safe === undefined)
                    diag.error(field + '.frames entry ' + JSON.stringify(String(entry)) + ' is not a safe image file name');
                else
                    frames.push(safe);
            }
            if (frames.length === raw.frames.length)
                track.frames = frames;
        }
    }
    ...
    if (raw.loop !== undefined) {
        if (typeof raw.loop !== 'boolean')
            diag.error(field + '.loop must be a boolean');
        else
            track.loop = raw.loop;
    }
    if (raw.fallback !== undefined) {
        if (typeof raw.fallback !== 'string' || !validTrackName(raw.fallback))
            diag.error(field + '.fallback must be a track name');
        else
            track.fallback = raw.fallback;
    }
...
function parseFrames2dBlock(raw, diag) {
    ...
    if (extra.length > 0)
        diag.error('frames2d: unknown field(s) ' + extra.map(k => JSON.stringify(k)).join(', '));
    const block = { tracks: {}, phases: { idle: '' } };
    if (raw.dir !== undefined) {
        const dir = safeManifestPath(raw.dir);
        if (dir === undefined)
            diag.error('frames2d.dir must be a safe manifest-relative directory');
        else
            block.dir = dir;
    }
    if (raw.defaultFrameMs !== undefined) {
        if (typeof raw.defaultFrameMs !== 'number' || !Number.isInteger(raw.defaultFrameMs) || raw.defaultFrameMs < FRAMES2D_MIN_FRAME_MS || raw.defaultFrameMs > FRAMES2D_MAX_FRAME_MS) {
            ...
            block.defaultFrameMs = raw.defaultFrameMs;
    }
    const tracks = {};
    if (!isRecord(raw.tracks)) {
        ...
        if (entries.length === 0)
            diag.error('frames2d.tracks must declare at least one track');
        if (entries.length > FRAMES2D_MAX_TRACKS)
            diag.error('frames2d.tracks declares too many tracks (' + entries.length + ', max ' + FRAMES2D_MAX_TRACKS + ')');
    ...
    if (raw.phases === undefined)
        diag.error('frames2d.phases is required (at least an "idle" mapping)');
    else if (phases !== undefined && phases.idle === undefined)
        diag.error('frames2d.phases.idle is required (unmapped phases fall back to it)');
    ...
    if (phasesValid) {
        for (const [phase, target] of Object.entries(block.phases)) {
            if (target !== undefined && tracks[target] === undefined)
                diag.error('frames2d.phases.' + phase + ' references unknown track ' + JSON.stringify(target));
        }
    }
...
function parseV2(source, diag) {
    ...
    if (renderer === 'sprite2d') {
        ...
        if (source.frames2d !== undefined)
            diag.error('renderer sprite2d must not declare a frames2d block');
    }
        ...
        if (source.frames2d !== undefined)
            diag.error('renderer live2d must not declare a frames2d block');
    }
    else if (renderer === 'frames2d') {
        const block = parseFrames2dBlock(source.frames2d, diag);
        if (block !== undefined)
            manifest.frames2d = block;
        if (source.sprite2d !== undefined)
            diag.error('renderer frames2d must not declare a sprite2d block');
        if (source.live2d !== undefined)
            diag.error('renderer frames2d must not declare a live2d block');
    }
