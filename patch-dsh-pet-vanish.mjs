...
if (src.includes('const handleHide = () =>')) {
  const upgrades = [
    ['for (let i = 0; i < 16; i++)', 'for (let i = 0; i < 40; i++)', '粒子数量 40'],
    ['const dist = 40 + Math.random() * 110;', 'const dist = 60 + Math.random() * 160;', '粒子扩散距离'],
    ['(Math.random() * 0.12) + "s"', '(Math.random() * 0.3) + "s"', '粒子随机延迟'],
    ['window.setTimeout(() => p.remove(), 1000);', 'window.setTimeout(() => p.remove(), 3000);', '粒子存活时长'],
    ['animation:dshPetParticle .8s ease-out forwards', 'animation:dshPetParticle 2.6s ease-out forwards', '粒子动画时长'],
    ['animation:dshPetDissolve .72s ease-in forwards', 'animation:dshPetDissolve 2s ease-in forwards', '消散动画时长'],
    ['props.onHide();\n\t\t\t\t}, 750);', 'props.onHide();\n\t\t\t\t}, 2200);', '隐藏延迟 2.2s'],
    ['props.onHide();\n\t\t\t\t}, 1500);', 'props.onHide();\n\t\t\t\t}, 2200);', '隐藏延迟 2.2s(旧版)'],
    ['animation:dshPetParticle 1.8s ease-out forwards', 'animation:dshPetParticle 2.6s ease-out forwards', '粒子动画时长(旧版)'],
    ['animation:dshPetDissolve 1.4s ease-in forwards', 'animation:dshPetDissolve 2s ease-in forwards', '消散时长(旧版)'],
    ['window.setTimeout(() => p.remove(), 2000);', 'window.setTimeout(() => p.remove(), 3000);', '粒子存活(旧版)'],
