...
if (src.includes('const handleHide = () =>')) {
  const upgrades = [
    ...
    ['window.setTimeout(() => p.remove(), 1000);', 'window.setTimeout(() => p.remove(), 2000);', '粒子存活时长'],
    ['animation:dshPetParticle .8s ease-out forwards', 'animation:dshPetParticle 1.8s ease-out forwards', '粒子动画时长'],
    ['animation:dshPetDissolve .72s ease-in forwards', 'animation:dshPetDissolve 1.4s ease-in forwards', '消散动画时长'],
    ['props.onHide();\n\t\t\t\t}, 750);', 'props.onHide();\n\t\t\t\t}, 1500);', '隐藏延迟 1.5s'],
  ];
