...
const VANISH_LOGIC = T([
  ...
  '\t\t\t\t\thost.appendChild(p);',
  '\t\t\t\t\twindow.setTimeout(() => p.remove(), 4000);',
  '\t\t\t\t}',
  '\t\t\t};',
  '\t\t\tconst handleHide = () => {',
...
if (src.includes('const handleHide = () =>')) {
  const upgrades = [
    ['for (let i = 0; i < 16; i++)', 'for (let i = 0; i < 48; i++)', '粒子数量 40'],
    ['const dist = 40 + Math.random() * 110;', 'const dist = 60 + Math.random() * 160;', '粒子扩散距离'],
    ['(Math.random() * 0.12) + "s"', '(Math.random() * 0.3) + "s"', '粒子随机延迟'],
    ['window.setTimeout(() => p.remove(), 1000);', 'window.setTimeout(() => p.remove(), 4000);', '粒子存活时长'],
    ['animation:dshPetParticle .8s ease-out forwards', 'animation:dshPetParticle 3.5s ease-out forwards', '粒子动画时长'],
    ['animation:dshPetDissolve .72s ease-in forwards', 'animation:dshPetDissolve 2s ease-in forwards', '消散动画时长'],
    ['props.onHide();\n\t\t\t\t}, 750);', 'props.onHide();\n\t\t\t\t}, 3200);', '隐藏延迟 2.2s'],
    ['props.onHide();\n\t\t\t\t}, 1500);', 'props.onHide();\n\t\t\t\t}, 3200);', '隐藏延迟 2.2s(旧版)'],
    ['animation:dshPetParticle 1.8s ease-out forwards', 'animation:dshPetParticle 3.5s ease-out forwards', '粒子动画时长(旧版)'],
    ['animation:dshPetDissolve 1.4s ease-in forwards', 'animation:dshPetDissolve 2s ease-in forwards', '消散时长(旧版)'],
    ['window.setTimeout(() => p.remove(), 2000);', 'window.setTimeout(() => p.remove(), 4000);', '粒子存活(旧版)'],
    ['dshPetParticle{0%{opacity:1;transform:translate(0,0) scale(1)}100%', 'dshPetParticle{0%{opacity:1;transform:translate(0,0) scale(1)}25%{opacity:1}100%', '粒子停留(旧版)'],
    ['dshPetParticle 2.6s ease-out forwards', 'dshPetParticle 3.5s ease-out forwards', '粒子3.5s(旧版)'],
    ['dshPetParticle{0%{opacity:1;transform:translate(0,0) scale(1)}25%{opacity:1}100%{opacity:0;transform:translate(var(--dx),var(--dy)) scale(.2)}}', 'dshPetParticle{0%{opacity:1;transform:translate(0,0) scale(1)}25%{opacity:1}55%{opacity:.92;transform:translate(calc(var(--dx)*.65),calc(var(--dy)*.5 - 36px)) scale(.55)}100%{opacity:0;transform:translate(calc(var(--dx)*1.1),calc(var(--dy) - 96px)) scale(.12)}}', '萤火轨迹(旧版)'],
    ['for (let i = 0; i < 40; i++)', 'for (let i = 0; i < 48; i++)', '粒子48(旧版)'],
    ['window.setTimeout(() => p.remove(), 3000);', 'window.setTimeout(() => p.remove(), 4000);', '粒子存活4s(旧版)'],
    ['props.onHide();\n\t\t\t\t}, 2200);', 'props.onHide();\n\t\t\t\t}, 3200);', '隐藏延迟3.2s(旧版)'],
    ['box-shadow:0 0 8px 2px rgba(140,255,200,.9)', 'box-shadow:0 0 12px 4px rgba(140,255,200,.95)', '光晕增强(旧版)'],
  ];
  for (const [oldS, newS, label] of upgrades) {
    if (src.includes(oldS)) { src = once(src, oldS, newS, label); applied.push(label) }
    else console.log('   跳过（已存在）: ' + label)
  }
} else {
